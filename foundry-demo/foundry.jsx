...
// ─────────────────────────────────────────────────────────────────────────
function FdyNode({ x, y, r, isStaff, infected, gold, infT, t, label }) {
  const fill = gold ? COLOR.gold : infected ? COLOR.alert : (isStaff ? COLOR.staff : COLOR.patient);
  const stroke = gold ? COLOR.gold : infected ? COLOR.alert : COLOR.mute;
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      ...
      {showGoldPulse && (
        <circle r={14} fill="none" stroke={COLOR.gold} strokeWidth="0.9" opacity="0.7">
...
function FoundryStack() {
  ...
  // Sub-timing
  const headerOp        = clamp(t / 0.6, 0, 1);
  const topPlatformsP   = clamp((t - 0.8) / 1.4, 0, 1);
  const topIconsP       = clamp((t - 2.0) / 1.0, 0, 1);
  const flowTopP        = clamp((t - 2.8) / 1.4, 0, 1);
  const midPlatformP    = clamp((t - 3.6) / 1.4, 0, 1);
  const wardsP          = clamp((t - 4.8) / 1.0, 0, 1);
  const dotsP           = clamp((t - 5.6) / 1.4, 0, 1);
  const bottomPlatformP = clamp((t - 13.5) / 1.4, 0, 1);
  const flowBottomP     = clamp((t - 14.3) / 1.2, 0, 1);
  const robotApppearP   = clamp((t - 15.0) / 0.8, 0, 1);
  const bubbleP         = clamp((t - 16.0) / 1.0, 0, 1);
  ...
  const bottomLogoSpin = t > 21
    ? Easing.easeInOutCubic(clamp((t - 21) / 1.0, 0, 1)) * 360
    : 0;
  const collapseP = clamp((t - 22) / 1.5, 0, 1);   // 0..1 over lt 22..23.5
  const collapseScale = 1 - collapseP * 0.95;       // 1 → 0.05
