...
function Stage({
  ...
  React.useEffect(() => {
    ...
    const onMsg = (e) => {
      const m = e && e.data;
      if (!m || typeof m !== 'object' || m.source !== 'nosotrack-host') return;
      switch (m.cmd) {
        case 'play':    setPlaying(true);  break;
        case 'pause':   setPlaying(false); break;
        case 'toggle':  setPlaying(p => !p); break;
        case 'seek':    setTime(clamp(Number(m.t) || 0, 0, duration)); break;
        case 'speed':   setSpeed(Number(m.value) || 1); break;
        case 'restart': setTime(0); setPlaying(true); break;
      }
    };
  ...
  React.useEffect(() => {
    ...
    if (now - lastBroadcastRef.current < 33 && playing) return;
    lastBroadcastRef.current = now;
    try {
      window.parent.postMessage({
        source: 'nosotrack-demo', type: 'tick',
        time, duration, playing, speed
      }, '*');
    } catch {}
